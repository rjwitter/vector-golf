document.addEventListener('DOMContentLoaded', () => {
    ...
    function draw() {
        ...
        // 7. Draw Tee Box (dynamic based on current level)
        const idx = (config.currentHole - 1) % LEVELS.length;
        const currentTee = LEVELS[idx].tee;
        ctx.beginPath();
        // Draw a 40x40 box centered on the tee coordinates
        ctx.rect(currentTee.x - 20, currentTee.y - 20, 40, 40);
